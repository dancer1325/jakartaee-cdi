...
* bean
    ** == 💡source of contextual objects / define application state and/or logic💡
        *** 👀contextual objects == object / EXIST | context / managed -- by -- CDI👀
    ** may bear
        *** metadata / define its lifecycle
        *** interactions -- , link:events.asciidoc[through events], with -- other beans
    ** == MULTIPLE attributes
        *** (nonempty) set of bean types
    ...
    ** types
        ...
        *** <<reserves,reserve>>
    ** 's instances are injected | somewhere -- based on, see <<injection_and_resolution>>, --
        *** bean types
        *** bean qualifiers

* _contextual instances of the bean_
    ** == PREVIOUS contextual objects
        *** ways
            **** `@Inject`
            **** `.get()`
    ** container
        *** creates these instances
        *** destroys these instances
        *** associates these instances -- with the -- appropriate context
    ** uses
        *** inject other objects (EVEN OTHER bean instances) / executed | SAME context

* bean developer
    ** provides
        *** the bean <<implementation>> -- by -- writing business logic | Java code
    ** may
        *** about interceptors
            **** create interceptors OR
            **** reuse existing interceptors
        *** | {cdi_full} environment, about <<decorators>>
            **** create decorators
...
* A bean type may be an interface, a concrete class or an abstract class, may be declared sealed or non-sealed or final,
and may have final methods.
...
Note that certain additional restrictions are specified in <<unproxyable>> for beans with a normal scope,
as defined in <<normal_scope>>.
...
If a bean class or producer method or field specifies a `@Typed` annotation, and the `value` member specifies a class
...
* _reserve_ is a bean that must be explicitly selected if it should be available for lookup, injection or name resolution.
